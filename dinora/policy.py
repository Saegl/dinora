...
move_lookup = {chess.Move.from_uci(move): i for move, i in zip(
    uci_labels, range(len(uci_labels)))}
flipped_move_lookup = {chess.Move.from_uci(move): i for move, i in zip(
    flipped_uci_labels, range(len(uci_labels)))}
